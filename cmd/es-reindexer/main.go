...
func createSelectUsersQuery(order string, limit string, condition string) string {
	...
	SELECT
		...
		u.name,
		u.username,
		if(last_login > '1970-01-01', last_login, '1970-01-01') last_login,
		if(u.signup > '1970-01-01', u.signup, '1970-01-01') signup,
		if(u.modified > '1970-01-01', u.modified, '1970-01-01') modified,
		u.sex,
		u.tz,
		(SELECT name    FROM gn_cities  WHERE gn_cities.id = u.city_home) city,
		(SELECT country FROM gn_countries WHERE gn_countries.iso2 = u.country_home) country,
		u.country_home,
		u.country_home iso2,
		u.website,
		u.main_photo_id,
